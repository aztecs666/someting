...
- **Global scope**: `E:/MLCollege/real data/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/MLCollege/real data/.opencode/notes/`)
...
2. Glob `E:/MLCollege/real data/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (E:/MLCollege/real data/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/MLCollege/real data/.opencode/notes/` — works in any directory
